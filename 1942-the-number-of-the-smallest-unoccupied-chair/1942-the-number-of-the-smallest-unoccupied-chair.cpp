...
public:
    int smallestChair(vector<vector<int>>& times, int targetFriend) {
        ...
        int chair = 0;
        set<int> releasedChairs;
        for (auto& time : times) {
            int start = time[0], end = time[1];
            while (!pq.empty() and pq.top().first <= start) { // release chairs based on arrival time of current friend
                releasedChairs.insert(pq.top().second);
                pq.pop();
            }
            int currChair;
            if (!releasedChairs.empty()) { // if chairs are free, get the first available chair
                currChair = *releasedChairs.begin();
                releasedChairs.erase(releasedChairs.begin());
